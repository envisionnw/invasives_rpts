dbMemo "SQL" ="SELECT l.Unit_Code, l.Visit_Year, l.Route, MIN(l.Area) AS Area, l.Transect, MIN("
    ...
    "er) AS TotalCover, MIN(ac.SampledQuadrats) AS QuadratsSampled, MIN(ac.TransectsS"
    ...
    "st AS l LEFT JOIN Transect_AverageCover AS ac ON (ac.IsDead = l.IsDead) AND (ac."
    "PlantCode = l.PlantCode) AND (ac.Transect = l.Transect) AND (ac.Route = l.Route)"
    " AND (ac.Visit_Year = l.Visit_Year) AND (ac.Unit_Code = l.Unit_Code)\015\012GROU"
