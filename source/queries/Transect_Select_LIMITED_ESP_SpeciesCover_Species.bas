dbMemo "SQL" ="SELECT DISTINCT ts.Unit_Code, ts.Visit_Year, ts.Location_ID, ts.Route, ts.Transe"
    ...
    "sDead, sc.PercentCover, sc.ID AS SpeciesCover_ID, (ts.Quadrat_ID & '-' & sc.ID) "
    "AS CountID\015\012FROM (Transect_Select_ESP AS ts LEFT JOIN SpeciesCover AS sc O"
    "N sc.Quadrat_ID = ts.Quadrat_ID) LEFT JOIN tlu_NCPN_Plants AS p ON p.Master_PLAN"
    "T_Code = sc.PlantCode\015\012ORDER BY ts.Route, ts.Transect, ts.Quadrat, IIf([Un"
    "it_Code] In (\"CARE\",\"DINO\",\"GOSP\",\"ZION\"),[Utah_Species],\015\012IIf([Un"
    "it_Code]=\"FOBU\",[WY_Species],[Co_Species]));\015\012"
dbMemo "Connect" =""
...
dbBinary "GUID" = Begin
    0xef91653cb9beaa4b959010b925f3994b
End
Begin
    Begin
