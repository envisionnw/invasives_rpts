dbMemo "SQL" ="SELECT DISTINCT (ts.ID & \"_\" & sc.PlantCode & \"_\" & sc.IsDead) AS ID, ts.Uni"
    "t_Code, ts.Visit_Year, ts.Location_ID, ts.Route, ts.Transect_ID, ts.Transect, ts"
    ".Area, ts.E_Coord, ts.N_Coord, ts.Quadrat_ID, ts.Quadrat, ts.IsSampled, ts.NoExo"
...
dbMemo "Filter" ="((([Transect_Select_LIMITED_ESP_SpeciesCover_Species].[Unit_Code]=\"GOSP\"))) AN"
    "D ([Transect_Select_LIMITED_ESP_SpeciesCover_Species].[Visit_Year]=2016)"
