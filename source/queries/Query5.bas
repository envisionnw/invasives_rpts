dbMemo "SQL" ="SELECT ts.Unit_Code, ts.Visit_Year, ts.Location_ID, ts.Route, ts.Transect, ts.Ar"
    ...
    "Unit_Code = ts.Unit_Code) AND (sc.Visit_Year = ts.Visit_Year) AND (sc.Route = ts"
    ".Route) AND (sc.Transect = ts.Transect) AND (sc.Area = ts.Area) AND (sc.E_Coord "
    "= ts.E_Coord) AND (sc.N_Coord = ts.N_Coord)\015\012GROUP BY ts.Unit_Code, ts.Vis"
...
dbMemo "OrderBy" ="[Query5].[Transect]"
Begin
    Begin
