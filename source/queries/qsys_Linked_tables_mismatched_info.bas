dbMemo "SQL" ="SELECT MSysObjects.Name AS CurrTable, tsys_Link_Tables.Link_table, MSysObjects.T"
    ...
    ".Link_table) ON tsys_Link_Dbs.Link_db = tsys_Link_Tables.Link_db\015\012WHERE MS"
    "ysObjects.Type NOT IN (-32761,-32764,-32768)\015\012AND\015\012(\015\012(((MSysO"
    "bjects.Type) In (4,6)) And ((IIf([Type]=4,ParseConnectionStr([Connect]),ParseFil"
    "eName([Database])))<>tsys_Link_Tables.Link_db)) Or (((MSysObjects.Type) In (4,6)"
    ") And ((IIf([Type]=4,ParseConnectionStr([Connect],'SERVER=')))<>[Server])) Or (("
    "(MSysObjects.Type) In (4,6)) And ((MSysObjects.Database)<>[File_path])) Or (((MS"
    "ysObjects.Type)=4) And ((tsys_Link_Dbs.Is_ODBC)=False)) Or (((MSysObjects.Type)="
    "6) And ((tsys_Link_Dbs.Is_ODBC)=True)) Or (((IIf([Type]=4,ParseConnectionStr([Co"
    "nnect],'SERVER='))) Is Null) And ((tsys_Link_Dbs.Server) Is Not Null)) Or (((IIf"
    "([Type]=4,ParseConnectionStr([Connect],'SERVER='))) Is Not Null) And ((tsys_Link"
    "_Dbs.Server) Is Null)) Or (((MSysObjects.Database) Is Null) And ((tsys_Link_Dbs."
    "File_path) Is Not Null)) Or (((MSysObjects.Database) Is Not Null) And ((tsys_Lin"
    "k_Dbs.File_path) Is Null))\015\012);\015\012"
dbMemo "Connect" =""
...
dbBinary "GUID" = Begin
    0x9eba268c56e5e745a52de4ec7c0df9cc
End
Begin
    Begin
        dbText "Name" ="CurrDb"
